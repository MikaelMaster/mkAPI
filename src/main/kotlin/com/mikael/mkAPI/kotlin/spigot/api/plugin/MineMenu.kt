...
import com.mikael.mkAPI.kotlin.spigot.api.modules.MineUtils
...
open class MineMenu(var title: String, var lineAmount: Int) : EventsManager() {
    ...
    var superiorMenu: MineMenu? = null


    var titleGenerated: java.util.function.Function<Player, String>? = null
    ...
    var openHandler: (MineMenu.(Inventory, Player) -> Unit)?
        set(value) {
    ...
    val playersMenu = mutableMapOf<Player, MineMenu>()

    var buildByPlayer: (MineMenu.(Player) -> Unit)? = null

    fun getMenu(player: Player): MineMenu {
        isPerPlayer = true
     ...
     * @param plugin Plugin
     */
    override fun register(plugin: MKPluginInstance) {
        registerMenu(plugin)
    }
     ...
     */
    open fun registerMenu(pluginInstance: MKPluginInstance) {
    ...
    companion object {
        ...
        val openIconDefault = Mine.newItem(
            Material.COMPASS, "§aMenu Exemplo",
            1, 0, "§2Clique abrir o menu"
        )
        fun debug(message: String) {
            if (isDebug) {
                MineUtils.console("§b[Menu] §7$message")
            }
        }
    }
